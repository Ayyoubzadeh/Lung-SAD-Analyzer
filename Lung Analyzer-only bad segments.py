...
class MainWindow(QDialog):
    ...
    def redraw(self,idx):
        ...
        self.pic43.setPixmap(pixMap4.scaledToHeight(PIC_HEIGHT)) 
    ...
    def draw3DSlow(self):
        ...
        mesh = Poly3DCollection(verts[faces], alpha=0.17)
                ...
                for i in range(mask.shape[0]):
                        for j in range(mask.shape[1]):
                            if mask[i,j]==True:
                                y.append(i)
                                x.append(j)
                                if reg_in[i,j]<-856 and reg_in[i,j]>-950 and ex[i,j]-reg_in[i,j]<60:
                                    c_y.append(i)
                                    c_x.append(j)   
                                    colors.append('red')  
                plt.scatter(c_x, c_y, c=colors, alpha=0.17,s=1)
                ...
                for i in range(mask.shape[0]):
                        for j in range(mask.shape[1]):
                            if mask[i,j]==True:
                                y.append(i)
                                x.append(j)
                                if reg_in[i,j]<-950 and ex[i,j]<-856:
                                    #c_y.append(i)
                                    #c_x.append(j)  
                                    #colors.append('red')
                                    continue
                                elif reg_in[i,j]>-950 and ex[i,j]<-856: #Air Trapping
                                    c_y.append(i)
                                    c_x.append(j)   
                                    colors.append('red') 
                                    continue
    ...
    def processImages(self):
        ...
        self.abnormal_in_voxels_p=np.count_nonzero(f_only_lungs_in[f_only_lungs_in<-950])/np.count_nonzero(f_only_lungs_in)
        self.abnormal_ex_voxels_p=np.count_nonzero(f_only_lungs_ex[f_only_lungs_ex<-856])/np.count_nonzero(f_only_lungs_ex)
        
        normal_in_voxels=f_only_lungs_in[np.bitwise_and(f_only_lungs_in<0,f_only_lungs_in>-950)]
        normal_ex_voxels=f_only_lungs_ex[np.bitwise_and(f_only_lungs_ex<0,f_only_lungs_ex>-856)]
        ...
        inrange_ex_voxels=f_only_lungs_ex[np.bitwise_and(f_only_lungs_ex<=-856,f_only_lungs_ex>=-950)]
        inrange_in_voxels=f_only_lungs_in[np.bitwise_and(f_only_lungs_in<=-856,f_only_lungs_in>=-950)]
        ...
        greater_than_min_ex_voxels=f_only_lungs_ex[f_only_lungs_ex>=-950]
        greater_than_min_in_voxels=f_only_lungs_in[f_only_lungs_in>=-950]
        ...
        self.emphysema_vx_cnt=0
        self.airTrapping_vx_cnt=0
        self.Amphisem_vx_cnt=0
        for p in np.argwhere(self.binary_ex_filter==1):
            x,y,z=p
            if self.voxels_registered_inhale_HUs[x,y,z]<-950 and self.voxels_ex_HUs[x,y,z]<-856:
                self.emphysema_vx_cnt=self.emphysema_vx_cnt+1
            elif self.voxels_ex_HUs[x,y,z]<-856:
                self.airTrapping_vx_cnt=self.airTrapping_vx_cnt+1
            else:
                self.normal_vx_cnt=self.normal_vx_cnt+1
            if self.voxels_registered_inhale_HUs[x,y,z]<-856 and self.voxels_ex_HUs[x,y,z]-self.voxels_registered_inhale_HUs[x,y,z]<60:
                self.ATI_vx_cnt=self.ATI_vx_cnt+1
            elif self.voxels_registered_inhale_HUs[x,y,z]>-950 and self.voxels_registered_inhale_HUs[x,y,z]<-856:
                self.ATI_normal_vx_cnt=self.ATI_normal_vx_cnt+1
            else:
                self.Amphisem_vx_cnt=self.Amphisem_vx_cnt+1
    ...
    def __init__(self):
        ...
        self.PRM34= QLabel(self)
        self.PRM34.setStyleSheet("background-color: #fef8dd")

        self.Exhale34= QLabel(self)
        self.Exhale34.setStyleSheet("background-color: #fef8dd")
        ...
        parent_layout.addWidget(self.Exhale34,3,4)
        parent_layout.addWidget(self.PRM34,3,5)
        parent_layout.addWidget(self.ATIMap35,3,6)
        ...
        parent_layout.addWidget(self.pic43,4,4)  
        parent_layout.addWidget(self.pic44,4,5)     
        parent_layout.addWidget(self.pic45,4,6)     
        ...
        self.PRM34.setText("PRM")
        self.PRM34.setFont(QFont('Comfortaa', 14))
        self.Exhale34.setText("Exhale")
        self.Exhale34.setFont(QFont('Comfortaa', 14))
